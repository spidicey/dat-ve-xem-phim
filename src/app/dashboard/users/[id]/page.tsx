...
import { fetchAdmin, fetchUser } from "@/lib/request";
...
import { z } from "zod";
import { AdminType, KhachHangType } from "../../../../../types";

const SingleUserPage = ({ params }: { params: any }) => {
  const { data: session } = useSession();
  // @ts-ignore
  const token = session?.user.accessToken;
  const [admin, setAdmin] = useState<KhachHangType | null>(null);
  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      const khData = await fetchUser(token, params.id);
      setAdmin(khData);
      reset({
        // reset form with fetched data
        ten: khData.ten,
        cccd: khData.cccd,
        gioiTinh: khData.gioiTinh,
        username: khData.account.tk,
        password: khData.account.mk,
        email: khData.account.email,
      });
    };
    fetchAdminData();
  ...
  const onSubmit = async (data: TAdminSchema) => {
    ...
    const response = await fetch(
      `http://localhost:8080/api/auth/khachHang/${admin?.id_kh}`,
      {
        method: "PUT",
